...
class Voyage extends CommonObject
{
	 ...
	 */
	public $table_element = 'clienjoyholidays_voyage';
	...
	/**
	 * @var string String with name of icon for voyage. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'voyage@clienjoyholidays' if picto is file 'img/object_voyage.png'.
	 */
	public $picto = 'voyage@clienjoyholidays';
	...
	public $fields=array(
		...
		'amount' => array('type'=>'price', 'label'=>'Amount', 'enabled'=>'1', 'position'=>40, 'notnull'=>1, 'visible'=>1, 'isameasure'=>'1', 'help'=>"Si non renseigné : <br/> - Prix rempli en fonction du dictionnaire prix par défaut par pays <br/> - Ou prix rempli avec le prix par defaut gloabl (parametre du module)", 'validate'=>'1',),
		...
		'fk_soc' => array('type'=>'integer:Societe:societe/class/societe.class.php', 'label'=>'Soc', 'enabled'=>'1', 'position'=>32, 'notnull'=>1, 'visible'=>1,),
	);
	public $rowid;
	public $ref;
	public $label;
	...
	//  */
	// public $table_element_line = 'clienjoyholidays_voyageline';
	...
	//  */
	// protected $childtablesoncascade = array('clienjoyholidays_voyagedet');
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->clienjoyholidays->voyage->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->clienjoyholidays->voyage->write))
		 || (!empty($conf->global->MAIN_USE_ADVANCED_PERMS) && !empty($user->rights->clienjoyholidays->voyage->voyage_advance->validate))))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$dirsource = $conf->clienjoyholidays->dir_output . '/voyage/' . $oldref;
				$dirdest = $conf->clienjoyholidays->dir_output . '/voyage/' . $newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->clienjoyholidays->dir_output . '/voyage/' . $newref, 'files', 1, '^' . preg_quote($oldref, '/'));
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/clienjoyholidays/voyage_card.php', 1) . '?id=' . $this->id;
	...
	*/
	...
	public function getNextNumRef()
	{
		...
		if (!empty($conf->global->CLIENJOYHOLIDAYS_VOYAGE_ADDON)) {
			...
			$file = $conf->global->CLIENJOYHOLIDAYS_VOYAGE_ADDON . ".php";
			$classname = $conf->global->CLIENJOYHOLIDAYS_VOYAGE_ADDON;
	...
	public function setPriceCountry()
	{
		global $conf;
