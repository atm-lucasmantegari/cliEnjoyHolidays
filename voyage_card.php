...
dol_include_once('/clienjoyholidays/lib/clienjoyholidays_voyage.lib.php');
...
$upload_dir = $conf->clienjoyholidays->multidir_output[isset($object->entity) ? $object->entity : 1].'/voyage';
...
if (empty($reshook)) {
	...
	$backurlforlist = dol_buildpath('/clienjoyholidays/voyage_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/clienjoyholidays/voyage_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
	...
	*/
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			print $formfile->showdocuments('clienjoyholidays:Voyage', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/clienjoyholidays/voyage_agenda.php', 1).'?id='.$object->id);
